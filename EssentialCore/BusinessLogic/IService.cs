...
namespace EssentialCore.BusinessLogic
{
    public interface IService<T> where T : IEntityBase
    {
        ...
        public Task<DataResult<List<T>>> Seek(T entity, UserCredit userCredit);

        public Task<DataResult<List<T>>> SeekByValue(string value, Info info, UserCredit userCredit);
